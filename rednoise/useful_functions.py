...
def smoothbkpl(x,p):
    ...
    """
    return p[0]*(x/p[1])**(-p[2])*(1/2*(1+(x/p[1])**(1/p[4])))**((p[2]-p[3])*p[4])
...
def break_po(x,p):
    ...
    :return:
    """
    return p[3]*x**(-1)*(1+(x/p[0])**p[1])**p[2]
...
def get_longhist_withgti(time,epoch_info,len_bin):
    gti=[]
    for i in range(len(epoch_info)):
        gti.append([epoch_info[i][0],epoch_info[i][1]])
    evt=EventList()
    evt.time=time
    evt.gti=gti
    lc_new=evt.to_lc(dt=len_bin)
...
def plot_psd(lc,norm='frac',show=0,ifexpTfilter=0):
    ...
    if ifexpTfilter:  # ifexpTfilter=T_tot
        ...
        ps.power=ps.power[index]
        ps.freq=ps.freq[index]
    xbar=np.mean(lc.counts)/lc.dt
    poisson=2/xbar
    fig, ax1 = plt.subplots(1, 1, figsize=(9, 6), sharex=True)
    ax1.loglog()
    ax1.step(ps.freq, ps.power, lw=2, color='blue')
    ax1.plot(ps.freq,np.zeros(len(ps.freq))+poisson,'--',color='grey')
    ax1.plot(ps.freq,np.zeros(len(ps.freq))+np.mean(ps.power),'--',color='orange')
