...
def get_time_series(T_START,N_cts,T_stop,period,amp, model,eqw=0.1):
    ...
    if model=='sin':
        ...
        i=0
        while i<len(t_sin):
            if (0.5 - 0.5 * eqw) < get_turns(t_sin[i], 707.) < (0.5 + 0.5 * eqw):
                rand = np.random.random(1)[0]
                if rand < 0.25:
                    t_sin = np.delete(t_sin, i)
                else:
                    i += 1
            else:
                i += 1
...
def write_result(dataname,cts_num,amp_num):
    ...
    amp_standard=1.0
    time = get_epoch_time_series(cts_rate = cts_rate_standard*cts_num, period = 554., amp =amp_standard*amp_num , model = 'sin')
    w_range = 2 * np.pi * np.arange(1. / 300, 1. /3000., 1e-7)
    starttime = datetime.datetime.now()
...
path_out='./simulation/simulation_LW_554/'
def get_result_fromid(dataname,cts_num,amp_num):
    ...
    np.savetxt(path_out+'result_{0}_{1}_dither/result_sim_{2}.txt'.format(str(cts_num),str(amp_num),str(dataname)), result,
