...
res=np.loadtxt(path+'result_1h_{0}.txt'.format(str(srcid[2])))
for i in range(3,len(srcid)):
    if os.path.exists(path+'result_1h_{0}.txt'.format(str(srcid[i]))):
        res_single=np.loadtxt(path+'result_1h_{0}.txt'.format(str(srcid[i])))
        res=np.row_stack((res,res_single))
        np.savetxt(path+ 'all_result_1h.txt', res, fmt='%10d %10.2f %10.5f %10.5f %10.5f %10d %10.5f %10.5f %10.5f')
