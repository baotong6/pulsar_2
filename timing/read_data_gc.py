...
def get_data(dataname):
    time=[];energy=[];obs_ID=[]
    #with open(path+'txt/'+dataname, 'r') as file_to_read:
    #with open(path + 'txt_2_8k/' + dataname, 'r') as file_to_read:
    with open(path + 'txt_all_obs/' + dataname, 'r') as file_to_read:
        while True:
    ...
    epoch = np.loadtxt(path + 'txt_all_obs/' + 'epoch_src_'+dataname)
    #epoch = np.loadtxt(path + 'txt/' + 'LW_epoch.txt')
