...
def get_time_series(T_START,N_cts,T_stop,period,amp, model,eqw=0.2):
    ...
    if model=='sin':
        ...
        i=0
        while i<len(t_sin):
            path_info = './simulation/dither_info/'
            info=np.loadtxt(path_info+'40_info.txt')
            time_info=info[:,0]
            frac_info=info[:,1]
            time_info2=np.abs(time_info-t_sin[i])
            frac_temp=frac_info[np.where(time_info2==np.min(time_info2))[0][0]]
            rand = np.random.random(1)[0]
            if rand > frac_temp:
...
def write_result(dataname,cts_num,amp_num):
    ...
    amp_standard=1.
    time = get_epoch_time_series(cts_rate = cts_rate_standard*cts_num, period =5072.97, amp =amp_standard*amp_num , model = 'sin')
...
path_out='./simulation/Pdet_LW/40_dither/'
