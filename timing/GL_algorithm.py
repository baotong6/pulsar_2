...
def compute_GL(Tlist, m_max=12, w_range=None, ni=10, parallel=False):
    ...
    if N > 0:
        ...
        m_opt = np.argmax(O1m[1:])  # find optimum bin number, i.e largest odds-ratio
...
def write_result(dataname):
    ...
    w_range = 2 * np.pi * np.arange(1. / 8000, 1. /4000., 1.e-6)
    starttime = datetime.datetime.now()
...
def get_result_fromid(id_range):
    ...
    result = np.column_stack((result_srcid, result_runtime, result_Prob, result_wpeak, result_period, result_mopt,
                              result_wconf_lo, result_wconf_hi,result_O_per_w,result_p_per_w))
    print(result)
    ...
    np.savetxt('result_1h-3h_{0}.txt'.format(id_range[0]), result,
               fmt='%10d %10.2f %10.5f %10.10f %10.5f %10d %10.10f %10.10f %10.5f %10.5f')

cand_id=np.loadtxt('cand_id.txt')
cand_id=cand_id.astype(int)

#get_result_fromid(cand_id[50:51])
get_result_fromid([1206])