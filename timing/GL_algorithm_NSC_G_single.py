...
# path_out='./xmm_CV/'
# path='./xmm_CV/'
path_out='/Volumes/pulsar/xmm_CV/result/'
...
def write_result(dataname):
    ...
    time=filter_energy(time,energy,[2000, 10000])
    time=filter_random_photon(time)
    epoch_file = path + 'epoch_' + str(dataname) + '.txt'
    w_range = 2 * np.pi * np.arange(1. / 7000, 1. /5000., 1.e-6)
    starttime = datetime.datetime.now()
    GL_R = compute_GL(time, epoch_file,w_range=w_range, m_max=20, parallel=True)
...
def get_result_fromid(id_range):
    ...
    if type(result_srcid[0])==str:
        result_srcid[0]=0
        for i in range(len(result_srcid)):
            result_srcid[i]=i
...
#np.savetxt('result_1h-3h_{0}.txt'.format(id_range[0]), result, fmt='%10d %10.2f %10.5f %10.10f %10.5f %10d %10.10f %10.10f %10.5f %10.5f')
    np.savetxt(path_out+'result_CV/'+'result_1h_{0}_{1}.txt'.format(id_range[0],id_range[-1]), result,
               fmt='%d %10.2f %10.5f %10.10f %10.5f %10d %10.10f %10.10f')

#cand_id=np.linspace(1,518,518)
# cand_id=np.loadtxt('G_src.txt')
# cand_id=cand_id.astype(int)
id_range=['HT_CAS_pn','OY_CAR_pn','QZ_VIR_pn','V893_SCO_pn','VW_HYI_pn']
