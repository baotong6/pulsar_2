...
def input_srcinfo():
    cat=fits.open(path_in+'cheng2019_Tuc.fit')
...
def main_process():
    ...
    # rocket.make_region_each_obs(path_in,path_out_reg,ra=[267.0753717],dec=[-24.71579714],wcsimage=wcsimage,obs_ID_all=obs_ID_all,
    #                             ecf=90,srcid=['out1'],multiple_src=0,bkg=1,single_name='out1')
    # rocket.make_epoch_file(obsid=obs_ID_all,inpath=path_in,outpath=path_out_txt,outname='NGC6304_epoch')
    epoch_info=np.loadtxt(path_out_txt+'NGC6304_epoch.txt')
    ecf=50
