...
path_in='/Volumes/pulsar/M30/merge_data/timing/'
path_out_reg='/Volumes/pulsar/M30/merge_data/timing/region_startover/'
path_out_txt='/Volumes/pulsar/M30/merge_data/timing/txt_startover/'
# obs_ID_all=[78,953,954,955,956,2735,3384, 2736,3385,2737,3386,2738,3387,16527,15747,16529,17420,15748,16528]
obs_ID_all=[18997, 2679, 20732, 20792, 20796, 20731, 20726, 20725, 20795]
...
#             15570,14468]
ra_center = 325.092167;dec_center = -23.179861;inter_radius = 21.61363636363636  ##角秒
# obs_ID_all=[11031]
# ra_center = 154.40343;dec_center = -46.41248;inter_radius = 60  ##角秒

def input_srcinfo():
    cat=fits.open(path_in+'M30_p50_i5_src_1_2_4_8.fits')
    ra = cat[1].data['RA']
    dec = cat[1].data['DEC']
...
def main_process():
    ...
    # rocket.make_epoch_file(obsid=obs_ID_all,inpath=path_in,outpath=path_out_txt,outname='M30_epoch')
    epoch_info=np.loadtxt(path_out_txt+'M30_epoch.txt')
    for id in obs_ID_all[0:4]:
