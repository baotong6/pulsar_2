...
def compute_GL(Tlist,epoch_info, m_max=20, w_range=None, ni=10, parallel=False):
    ...
    if N > 0:
        ...
        wr = np.extract(np.logical_and(cdf > 0.16, cdf < 0.84), w)
...
def write_result(dataname):
    ...
    (useid, epoch_info_use)=hawk.choose_obs(epoch_info,flux_info=CR,
                                            ...
                                            if_flux_high=0, if_expT_high=1,obsID=[12110])
    ...
    counts=len(time)
    w_range=2*np.pi*np.arange(1./80,1./50,1.e-6)
    starttime = datetime.datetime.now()
...
def get_result_fromid(id_range):
    ...
    #                           result_wconf_lo, result_wconf_hi,result_counts))
    print(result)
    print(result_period[0],result_period[0]-2*np.pi/result_wconf_hi[0],2*np.pi/result_wconf_lo[0]-result_period[0])
