...
#path = '/Users/baotong/Desktop/period_terzan5/txt_all_obs_0.5_8/'
...
def write_result(dataname):
    ...
    w_range = 2 * np.pi * np.arange(1. /30000, 1./20000., 1.e-8)
    #w_range=2*np.pi*np.linspace(1./10000,1./8000.,2000)
    starttime = datetime.datetime.now()
...
def get_result_fromid(dataname):
    ...
    print(result)
    np.savetxt(path+'result_1h_{0}.txt'.format(str(dataname)), result,
               fmt='%10d %10.2f %10.5f %10.10f %10.5f %10d %10.10f %10.10f %10d')

get_result_fromid('91')
