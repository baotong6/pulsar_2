...
def get_time_series(T_START,N_cts,T_stop,period,amp, model):
    #N_cts为总光子数
    #T为总观测时间
    #period为周期
...
def write_result(dataname,cts_num,amp_num):
    ...
    amp_standard=1.
    time = get_epoch_time_series(cts_rate = cts_rate_standard*cts_num, period = 5540., amp =amp_standard*amp_num , model = 'sin')
