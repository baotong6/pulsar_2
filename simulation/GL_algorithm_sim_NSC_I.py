...
def get_epoch_time_series(cts_rate,period,amp, model):
    ...
    if model=='const':
        for i in range(len(tstart)):
            ...
            #这个数字是根号10和根号10分之1，# 在10倍以内变化的cts-rate!!!
...
def write_result(dataname,cts_num,amp_num):
    ...
    w_range = 2 * np.pi * np.arange(1. / 60000, 1. /50000., 1e-9)
    starttime = datetime.datetime.now()
...
path_out='./simulation/simulation_NSC_I_step/'
def get_result_fromid(dataname,cts_num,amp_num):
    ...
    np.savetxt(path_out+'result_{0}_{1}_60k/result_sim_{2}.txt'.format(str(cts_num),str(amp_num),str(dataname)), result,
