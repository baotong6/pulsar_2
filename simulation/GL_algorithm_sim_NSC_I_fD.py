...
def get_time_series(T_START,N_cts,T_stop,period=50000.,amp=0.5, model='const',eqw=0.1):
    #N_cts为总光子数
    #T为总观测时间
    #period为周期
...
def get_epoch_time_series(cts_rate,period,amp, model):
    ...
    if model=='const':
        for i in range(len(tstart)):
            cts = int((cts_rate * (tstop[i] - tstart[i]))*(np.random.random(1)[0]*3.1622776+0.316227766))
            # 在10倍以内变化
            t.append(get_time_series(tstart[i], cts, tstop[i], period, amp, model=model))
    else:
        for i in range(len(tstart)):
            cts = int(cts_rate * (tstop[i] - tstart[i]))
            t.append(get_time_series(tstart[i],cts,tstop[i],model=const))
    t=list(chain.from_iterable(t))
    t = np.array(t)
    t = t + np.random.rand(len(t)) * 3.2 - 1.6
    return t

path='./period_NSC_merge_IG/txt_all_obs_I/'
...
path_out='./simulation/sim_NSC_pCV_fD/'
...
def write_result(dataname):
    ...
    amp_standard=1.0
    #time = get_epoch_time_series(cts_rate = cts_rate_standard*cts_num, period =10*86400*365, amp =amp_standard*amp_num , model = 'const')
    time=get_sim_time_series(dataname,mode='CTS')
    w_range = 2 * np.pi * np.arange(1. / 50000, 1. /40000., 1e-9)
    starttime = datetime.datetime.now()
...
def get_result_fromid(sim_id,dataname):
    ...
    np.savetxt(path_out+'result_{0}_50k/result_sim_{1}.txt'.format(str(dataname),str(sim_id)), result,
               fmt='%10d %10.2f %10.5f %10.10f %10.5f %10d %10.10f %10.10f %10d')


import sys
if __name__ == '__main__':
    get_result_fromid(sys.argv[1],sys.argv[2])
    sys.exit(0)
