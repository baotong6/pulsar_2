...
def get_LS(time, flux,freq,trial=1,smooth=100):
    path='/Users/baotong/Desktop/CDFS/txt_all_obs_0.5_8_ep3/simulation/89_LS_sim_noQPO/'
    x = time
    y = flux
    # dy=np.sqrt(y)
    # plt.scatter(x,y)
...
def get_lc_onesource(k,src_index,num_trials=2,smooth=100):
    k_trial =0
    FP = [];
    period = [];
    cts_num=[];
    ...
    while k_trial <num_trials:
        ...
        freq=np.arange(1/T_exp,0.5/dt,1/(5*T_exp))
        freq=freq[np.where(freq > 1 / 20000.)]
        temp=get_LS(lc_new.time, lc_new.counts, freq=freq,trial=k_trial,smooth=smooth)
        res[k_trial]=temp[-1]
        k_trial+=1
    ...
    np.savetxt(path + 'simulation/89_LS_sim_noQPO/' + 'trial_out_src{0}_REJ1034+396_noQPO.txt'.format(src_index), res,
               fmt="%10.5f")
    # np.save(path + 'simulation/89_LS_sim_noQPO/' + 'trial_out_src{0}_REJ1034+396_noQPO.txt'.format(src_index), res)
get_lc_onesource(3,'89',num_trials=5)
