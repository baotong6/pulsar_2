...
def get_T_in_mbins(epoch_file,w,m,fi):
    ...
    if type(epoch_info[0])==np.float64:
...
def write_result(dataname):
    ...
    w_range = 2 * np.pi * np.arange(1. /20000, 1. /3000., 1.e-7)
    starttime = datetime.datetime.now()
    GL_R = compute_GL(time, epoch_file,w_range=w_range, m_max=12, parallel=False)
...
def get_result_fromid(dataname):
    #id_all=np.loadtxt(path+'cand_other500.txt')
    result_srcid=int(dataname)
    #dataname=str(int(dataname))+"_keep"
    #dataname=int(dataname)
    #dataname = int(id_all[int(dataname)])
    ...
    np.savetxt(path_out+'result_all_obs_0.5_8_ep1/result_1h_3h_{0}.txt'.format(str(dataname)), result,
               fmt='%10d %10.2f %10.5f %10.10f %10.5f %10d %10.10f %10.10f %10d')

# import sys
# if __name__ == '__main__':
#     get_result_fromid(sys.argv[1])
cand_id=np.linspace(1,2,2)
cand_id=cand_id.astype(int)
for i in range(len(cand_id)):
    get_result_fromid(cand_id[i])
