...
def plot_obs_epoch(epoch_file,color='red',epoch_days='65 days'):
    ...
    plt.plot([x[0]-50,x[0]-50],[0,100],'--',color=color)
    plt.plot([x[-1]+50, x[-1]+50], [0, 100], '--', color=color)
    # plt.annotate(s='', xy=(1, 1), xytext=(0, 0), arrowprops=dict(arrowstyle='<->'))
    plt.annotate("", xy=(x[0]-50, 100),
                xytext=(x[-1]+50, 100), color=color,
                weight="bold",
                arrowprops=dict(arrowstyle="<->", connectionstyle="arc3", color=color))
    plt.text(x[-1]+50,100,epoch_days,color=color,fontsize=12)
...
if __name__=='__main__':
    ...
    for i in range(4):
        epoch_file='/Users/baotong/Desktop/CDFS/txt_all_obs_0.5_8_ep{0}/epoch_src_89.txt'.format(i+1)
