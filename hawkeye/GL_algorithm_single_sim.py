...
#     # path_Tuc = '/Users/baotong/Desktop/period_NGC6397/txt_all_obs_p90/'
...
#     path=path_Tuc
#     epoch_file='epoch_src_245.txt'
#     cts_rate_standard=0.0001
#     amp_standard=1.0
#     time = hawk.get_epoch_time_series(cts_rate = cts_rate_standard*cts_num, period =36000., amp =amp_standard*amp_num , model = 'sin')
#
#     for id in id_range:
#         dataname=str(id)
...
def get_result_fromid(path_out,dataname,cts_num,amp_num):
    ...
    result_wmean=res[5]
    result_mopt=res[6]
    result_wconf_lo=res[7]
    result_wconf_hi=res[8]
    result_N_cts=res[9]
    result = np.column_stack((result_srcid, result_runtime, result_Prob,result_period, result_wpeak, result_wmean, result_mopt,
    ...
    for cts_num in cts_range:
        for amp_num in amp_range:
            for i in range(len(id_range)):
                get_result_fromid(path_out,id_range[i],cts_num=cts_num,amp_num=amp_num)
